...
async def git_push_kernel_source(client, message):
    ...
    if "Kernel.tar.gz" not in extracted_files or "Platform.tar.gz" not in extracted_files:
    ...
    if os.path.exists(f"{GIT_BRANCH_NAME}/drivers/misc/mediatek/connectivity"):
        ...
        subprocess.run([f"tar", "-xzvf", os.path.join(EXTRACT_PATH/Platform.tar.gz"), "-C", "Platform"])
