...
async def editMessage(message, text):
    try:
        ...
        print(f"Error in editMessage: {e}")


async def git_push_kernel_source(client, message):
    try:
        if not message.reply_to_message or not message.reply_to_message.document:
            await sendMessage(message, "<b>Error:</b> Please reply to a zip file with the command.")
            return

        args = message.text.split()
        if len(args) < 2:
            await sendMessage(message, "<b>Error:</b> Please provide a branch name.")
            return
        GIT_BRANCH_NAME = args[1]

        file_name = message.reply_to_message.document.file_name
        if not file_name.endswith(".zip"):
            await sendMessage(message, "<b>Error:</b> Only zip files are supported.")
            return
        ...
        try:
            ...
            subprocess.run(f"7z x '{file_path}' -o'{extract_path}' -y", shell=True, check=True)
            os.remove(file_path)
            banner += " ☑️"
            await editMessage(status, banner)
        except Exception as e:
        ...
        try:
            ...
            subprocess.run(["git", "config", "--global", "init.defaultBranch", GIT_BRANCH_NAME], check=True)
        ...
        try:
            ...
            subprocess.run(["git", "branch", "-M", f"{GIT_BRANCH_NAME}"], cwd=f"{GIT_BRANCH_NAME}", check=True)
            banner += " ☑️"
            await editMessage(status, banner)
        except Exception as e:
            banner += "\n\n<b>Error:</b> {e}.</b>"
        ...
        try:
            ...
            await editMessage(status, banner)
            subprocess.run(["git", "remote", "add", "origin", f"{GIT_REMOTE_ORIGIN}"], cwd=f"{GIT_BRANCH_NAME}", check=True)
            subprocess.run(["git", "push", "-u", "origin", f"{GIT_BRANCH_NAME}"], cwd=f"{GIT_BRANCH_NAME}", check=True)
            banner += " ☑️"
            await editMessage(status, banner)
        except Exception as e:
            banner += "\n\n<b>Error:</b> {e}.</b>"
